...
export function useDashboardStats() {
  const { data: automobiliai = [], isLoading: loadingA } = useGetAllCarsApiV1CarsGetQuery();
  const { data: uzsakymai = [], isLoading: loadingU } = useGetAllOrdersApiV1OrdersGetQuery();
  const { data: rezervacijos = [], isLoading: loadingR } = useGetAllReservationsApiV1ReservationsGetQuery();
  const { data: klientai = [], isLoading: loadingK } = useGetAllClientsApiV1ClientsGetQuery();
  const { data: saskaitos = [], isLoading: loadingS } = useGetAllInvoicesApiV1InvoicesGetQuery();
  const { data: pagalbosUzklausos = [], isLoading: loadingP } = useGetAllSupportsApiV1SupportGetQuery();

  const isLoading = loadingA || loadingU || loadingR || loadingK || loadingS || loadingP;
  // Greitesnis lookup per Map
  const automobiliaiMap = useMemo(() => new Map(automobiliai.map(a => [a.automobilio_id, a])), [automobiliai]);
  const klientaiMap = useMemo(() => new Map(klientai.map(k => [k.kliento_id, k])), [klientai]);
  ...
  const barData = useMemo(() => [
    { name: "Patvirtinti", value: uzsakymai.filter(u => u.uzsakymo_busena === "patvirtinta").length },
